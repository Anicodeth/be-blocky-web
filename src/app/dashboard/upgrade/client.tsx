...
export const UpgradeTab = () => {
    ...
    return (
        <Tabs aria-label="Options" variant="solid" size="lg" radius="full" classNames={{
            ...
            <Tab key="/monthly" title="Monthly">
                <div className=" flex flex-col md:flex-row md:items-center gap-12 py-6 md:py-20">
                    {plans.map(plan => {
                        const isCurrent = plan.name.toLowerCase() === userData?.subscription.toLowerCase()
                        return (
                            <Card key={plan.name} className={cn("flex-grow flex flex-col justify-between text-xs md:text-base", plan.name === "Standard" && " border border-brand-orange/20 ")}>
                                ...
                                <div>
                                    ...
                                    <CardFooter>
                                        <Button className="text-xs md:text-base" onClick={() => subscribeToPlan(plan)} disabled={isLoading || isCurrent}>
                                            Choose Plan
                                        </Button>
                                    </CardFooter>
                                </div>
