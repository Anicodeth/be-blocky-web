...
export default function page() {
    ...
    async function login(data: SignInSchema) {
        ...
        await signInWithEmailAndPassword(auth, email, password).then(async (userCred) => {
            ...
            title: e?.message ?? "Error happened while signing in, Please try again!"
        }))
        setIsLoading(false)
    }
    return (
        <div>
            <Form {...form}>
                <form className=" space-y-4" onSubmit={form.handleSubmit(login, errorToast)}>
                    <FormField
                    ...
                    <FormField
                        ...
                        render={(field) => (
                            <FormItem>
                                <FormControl>
                                    <Input {...field.field} placeholder="Password" type="password" className=" h-10" />
                                </FormControl>
                            </FormItem>
                        )}
                    />
